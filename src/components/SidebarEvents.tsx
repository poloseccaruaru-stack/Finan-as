...
import { Calendar, Bell, Plus, X, Info, Settings, ArrowUp, ArrowDown, Save, Clock } from 'lucide-react';
import { format, isToday, isAfter, startOfDay, parseISO } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { CalendarEvent, Teacher, DashboardConfig } from '../types';
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-slate-900 text-sm tracking-tight">Novos Eventos</h3>
              ...
              <AnimatePresence>
                {showPositionConfig && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 10 }}
                    className="absolute left-0 bottom-full mb-2 bg-white rounded-xl shadow-2xl p-3 border border-slate-100 min-w-[160px] z-[110]"
                  >
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Posição da Barra</p>
                    <div className="space-y-1">
                      <button 
                        onClick={async () => {
                          const docRef = doc(db, 'config', 'dashboard');
                          await updateDoc(docRef, { eventBarPosition: 'top' });
                          setShowPositionConfig(false);
                        }}
                        className={cn(
                          "w-full flex items-center justify-between px-3 py-2 rounded-lg text-[10px] font-bold uppercase transition-all",
                          config?.eventBarPosition === 'top' ? "bg-indigo-50 text-indigo-700" : "hover:bg-slate-50 text-slate-600"
                        )}
                      >
                        Superior
                      ...
                      <button 
                        onClick={async () => {
                          const docRef = doc(db, 'config', 'dashboard');
                          await updateDoc(docRef, { eventBarPosition: 'bottom' });
                          setShowPositionConfig(false);
                        }}
                        className={cn(
                          "w-full flex items-center justify-between px-3 py-2 rounded-lg text-[10px] font-bold uppercase transition-all",
                          (!config || config?.eventBarPosition === 'bottom') ? "bg-indigo-50 text-indigo-700" : "hover:bg-slate-50 text-slate-600"
                        )}
                      >
                        Inferior
